...
namespace Luna
{
	...
	HashMap<u64, NameEntry> g_name_map;
	handle_t g_name_mtx;
	...
	LUNA_RUNTIME_API const c8* intern_name(const c8* name)
	{
		...
		auto iter = g_name_map.find(h);
		if (iter != g_name_map.end())
		{
		...
		else
		{
			...
			*h_buf = h;
			strcpy(buf, name);
			g_name_map.insert(make_pair(h, NameEntry(buf, 1)));
			OS::unlock_mutex(g_name_mtx);
			return buf;
		}
	}
	...
	LUNA_RUNTIME_API void retain_name(const c8* name)
	{
		if (!name) return;
		OS::lock_mutex(g_name_mtx);
		auto iter = g_name_map.find(get_name_id(name));
		if (iter == g_name_map.end())
		{
			OS::unlock_mutex(g_name_mtx);
			return;
