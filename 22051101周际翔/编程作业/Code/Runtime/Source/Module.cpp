...
namespace Luna
{
	...
	{
		lucheck_msg(!g_engine_initialized, "remove_engine_module must be called before `Luna::init` or after `Luna::close`.");
		ModuleEntry* entry = (ModuleEntry*)(module_desc->reserved);
	...
	LUNA_RUNTIME_API RV init_modules()
	{
		...
		while (!unresolved_modules.empty())
		{
			...
			while (iter != unresolved_modules.end())
			{
				...
				// Check dependencies.
				auto str = (*iter).second->dependencies;
				while (*str && strcmp(*str, u8""))
				{
					Name dep(*str);
