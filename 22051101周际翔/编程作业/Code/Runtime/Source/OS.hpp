...
namespace Luna
{
	namespace OS
	{
		...
		//! Suspends the current fiber context and switches to the target fiber context. This 
		...
		//! @return Returns the pointer set.
		...
		R<handle_t> open_file(const c8* path, EFileOpenFlag flags, EFileCreationMode creation);
		...
		RV write_file(handle_t file, void* buffer, usize size, usize* write_bytes = nullptr);
		...
		RV set_file_cursor(handle_t file, i64 offset, ESeekMode mode);
		...
		//! * BasicError::not_found
		//! * BasicError::bad_system_call for all errors that cannot be identified.
		R<handle_t> open_buffered_file(const c8* path, EFileOpenFlag flags, EFileCreationMode creation);
		...
		//! already be written while others are not, in such case the `write_bytes` reported by system may not be equal to `size` 
		//! specified by the user.
		RV write_buffered_file(handle_t file, void* buffer, usize size, usize* write_bytes = nullptr);
		...
		//! 
		//! The cursor value cannot be negative, if the new cursor position goes below 0, the seek operation fails.
		RV set_buffered_file_cursor(handle_t file, i64 offset, ESeekMode mode);
