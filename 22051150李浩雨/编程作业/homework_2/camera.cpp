...
void Camera::printFrontModel(vector<Model>& models)
{
	int index = -1;
	float dotMin = 0;
	for (int i = 0; i < models.size(); i++) {
		glm::vec3 dirToModel = models[i].getPosition() - Position;
		float distance = glm::length(dirToModel);
		float temp = glm::dot(Forward, glm::normalize(dirToModel));
		//printf("%f %f ", distance, temp);
		if (temp > 0 && temp > dotMin && distance < 100) {
			dotMin = temp;
			index = i;
		}
	}
	if (index != -1) {
		printf("%s\n", models[index].getName().c_str());
	}
	else {
		printf("no models at a very small distance.\n");
	}
}
